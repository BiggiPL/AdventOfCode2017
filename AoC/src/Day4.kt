val contestValue = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
class Day4_1(private val input: String) {

    init {
        println("unique passphrases ${checkUniqueSets()}")
    }

    fun checkUniqueSets(): Int {
        return input.split("\n")
                .filter { s ->
...
class Day4_2(private val input: String) {

    init {
        println("non anagram passphrases ${nonAnagramCount()}")
    }

    fun nonAnagramCount(): Int {
        return input.split("\n")
                .filter { s -> isValidLine(s) }.size
    }

    fun isValidLine(line: String): Boolean {
        val words = line.split(" ")

        words.forEachIndexed { i, w: String ->
            val sorted = splitAndOrder(w)

            words.forEachIndexed { j, word ->
                if (i != j) {
                    if (sorted.contentEquals(splitAndOrder(word))) {
                        return false
                    }
                }
            }
    ...
    fun splitAndOrder(word: String): String {
        return word.split("")
