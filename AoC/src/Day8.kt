...
class Day8_1(input: String) {

    init {
        val instructions = input.split("\n").map { s: String -> Instruction(s) }

        val register = hashMapOf<String, Int>()
...
//            println(register)
        ...
        println(register.values.max())
    }

}

class Day8_2(input: String) {
    ...
    init {
        ...
        instructions.forEach { i ->
            i.operate(register)
//            println(register)
            max = kotlin.math.max(max, register.values.max()!!)
        }
...
class Instruction(private val text: String) {

    val targetRegister: String
    val operation: (Int, Int) -> Int
    val operationValue: Int
    val registerToCompare: String
    val comparator: (Int, Int) -> Boolean
    val valueToCompare: Int

    init {
        val split = text.split(" ")

        targetRegister = split[0]
        operationValue = split[2].toInt()
        registerToCompare = split[4]
        valueToCompare = split[6].toInt()

        operation = when (split[1]) {
            "inc" -> { a, b -> a + b }
            else -> { a, b -> a - b }
        }
        comparator = when (split[5]) {
            "==" -> { a, b -> a == b }
            "!=" -> { a, b -> a != b }
            ">" -> { a, b -> a > b }
            ">=" -> { a, b -> a >= b }
...
//        println("$targetRegister, $operationValue, $registerToCompare, $valueToCompare, $operation, $comparator")
    }

    fun operate(register: HashMap<String, Int>) {
        if (!register.contains(targetRegister)) {
            register[targetRegister] = 0
        }
        if (!register.contains(registerToCompare)) {
            register[registerToCompare] = 0
        }

        if (comparator(register[registerToCompare]!!, valueToCompare)) {
            register[targetRegister] = operation(register[targetRegister]!!, operationValue)
        }
    }
...
val example_8 = """b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"""

val contest_8 = """gug dec 188 if zpw >= 8
