...
class Day7_1(input: String) {

    val nodes = input.split("\n")

    init {
        println("root node: ${findRoot()}")
    }

    fun findRoot(searchIndex: Int = 0): String {
        val searchedValue = nodes[searchIndex].split(" ")[0]

        nodes.forEachIndexed { index, s ->
            if (index != searchIndex && s.contains(searchedValue)) {
...
class Day7_2(input: String) {

    val nodes: List<Node> = input.split("\n")
            .map { s -> Node(s) }

    init {
        nodes.forEach { node -> node.registerChildNodes(nodes) }

        val root = nodes.find { node -> node.parentNode == null }

        if (root != null) {
            checkIfWeightAreOk(root)
        } else {
            print("Root is null")
        }
    }

    fun checkIfWeightAreOk(node: Node, level: Int = 0) {
        val childrenOk = checkChildrenWeight(node.childNodes)

        println("\n${node.name} - $childrenOk")
        if (!childrenOk) {
            node.childNodes.forEach { node -> checkIfWeightAreOk(node, level + 1) }
        } else {
            val parent = node.parentNode
            println("${node.name} - ${node.parentNode}")
            if (parent != null) {
                val weightsOfNodes = getWeightsOfNodes(parent.childNodes)
                weightsOfNodes.forEach { t, u -> u.forEach { node -> print("$t - ${node.name}; ") } }

                var brokenWeight: Int = -1
                weightsOfNodes.keys.forEach { i: Int ->
                    if (weightsOfNodes[i]?.size == 1) {
                        brokenWeight = i
                    }
                }

                println(brokenWeight)

                if (brokenWeight > 0) {
                    var weights: Set<Int> = weightsOfNodes.keys
                    weights -= brokenWeight

                    println("Broken: $brokenWeight - Remain weights: $weights")
                    val difference = weights.first() - brokenWeight

                    if (weightsOfNodes[brokenWeight] != null) {
                        val weight = weightsOfNodes[brokenWeight].orEmpty()[0].weight

                        println("Weight: $weight - Target weight ${weight + difference} - $level (Highest level wins)")
                    }
                }
    ...
    fun checkChildrenWeight(children: List<Node>): Boolean {

        val map: Map<Int, List<Node>> = getWeightsOfNodes(children)

        return map.size == 1
    }

    fun getWeightsOfNodes(nodes: List<Node>): Map<Int, List<Node>> {

        val map: HashMap<Int, ArrayList<Node>> = hashMapOf()

        nodes.forEach { node ->
            val nodeWeight = node.getEntireWeight()

            if (!map.containsKey(nodeWeight)) {
                map.put(nodeWeight, arrayListOf())
            }

            map[nodeWeight]?.add(node)
        }
...
class Node(initialString: String) {
    ...
    init {
        val afterSplit = initialString.split("->")
        val nameAndWeight = afterSplit[0]
                .trim()
                .split(" ")
        ...
        children = if (afterSplit.size > 1) afterSplit[1].split(",").map { s -> s.trim() } else emptyList()

//        println("$name - $weight - $children")
    }

    fun registerChildNodes(nodes: List<Node>) {
        childNodes = nodes.filter { node -> children.contains(node.name) }
        parentNode = nodes.find { node -> node.children.contains(name) }
    }

    fun getEntireWeight(): Int {
        return weight + childNodes.sumBy { node -> node.getEntireWeight() }
    }

}

val example_7 = """pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
